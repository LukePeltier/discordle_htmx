...
func (mq *MessageQuery) QuerySender() *DiscordMemberQuery {
	query := (&DiscordMemberClient{config: mq.config}).Query()
...
func (mq *MessageQuery) WithSender(opts ...func(*DiscordMemberQuery)) *MessageQuery {
	query := (&DiscordMemberClient{config: mq.config}).Query()
...
func (mq *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if query := mq.withSender; query != nil {
		if err := mq.loadSender(ctx, query, nodes, nil,
			func(n *Message, e *DiscordMember) { n.Edges.Sender = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MessageQuery) loadSender(ctx context.Context, query *DiscordMemberQuery, nodes []*Message, init func(*Message), assign func(*Message, *DiscordMember)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Message)
	for i := range nodes {
		if nodes[i].discord_member_messages == nil {
			continue
		}
		fk := *nodes[i].discord_member_messages
	...
	query.Where(discordmember.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "discord_member_messages" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
